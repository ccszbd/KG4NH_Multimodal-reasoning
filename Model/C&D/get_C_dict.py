...
# else:
# E: whole entity name list(such as disease-011) len(E):19416
E = list(pd.read_csv('../../Dataset/parameter/entity.csv')['E'])
...
for e in E:
    ...
    c1, c2, c3 = E_dict[e]['C']
    # c_count: count each category type and its numbers len(c_count):26630
    for c in c2 + c3:
        if c not in c_count:
            c_count[c] = 1
        else:
            c_count[c] += 1
    # C: each entity has a category description, len(C):19416
    C.append([c1, c2, c3])
